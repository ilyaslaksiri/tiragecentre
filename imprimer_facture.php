...
$requete1="select * from FACTURE where ID_FACTURE='$id_facture'";
$result1= @mysql_query($requete1, $idcom);
$ligne1=mysql_fetch_array($result1);

$id_client=$ligne1['ID_CLIENT'];
$num_facture=$ligne1['NUM_FACTURE'];
$date_facture=$ligne1['DATE_FACTURE'];
$total_ttc=$ligne1['MONTANT_TTC'];
$total_ht=$ligne1['MONTANT_HT'];
$tva=$ligne1['TVA'];
$somme=$ligne1['MONTANT_LETTRE'];
$BL=$ligne1['BL'];
$etat_facture=$ligne1['ETAT_FACTURE'];
$mode_payement=$ligne1['MODE_PAYEMENT'];
...
$pdf->fact_dev( "Facture  ", $num_facture );

//$pdf->temporaire( "Devis temporaire" );
$pdf->addDate( $date_facture);
//$pdf->addClient("Ihsane laksiri");
//$pdf->addPageNumber("1");
//$pdf->addClientAdresse("33 Res yassine apt 3 Av des FAR Meknes VN");

if($etat_facture=="PAYE"){
$pdf->addReglement( utf8_decode($mode_payement));
}
...
$line = array( 
			   ...
			   "QUANTITE"      => " ",
               "DESIGNATION"  => " ".addslashes("BL N°: ".$BL),
			   "Prix HT" => " " ,
			   "Total HT" => " "
               );

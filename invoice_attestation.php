...
function addDevis( $numdev )
{
	...
	$this->fact_dev( "Bon de commande", $string );
}
...
function addDate( $date )
{
	...
	$this->Cell(10,5, "Meknès le:", 0, 0, "C");
...
{
	$this->SetFont( "Arial", "", 10);
	$length = $this->GetStringWidth( "Arreter le présent Bon de commande à la somme de: " . $remarque );
	$r1  = 10;
	$r2  = $r1 + $length;
	$y1  = $this->h - 90.5;
	$y2  = $y1+5;
	$this->SetXY( $r1 , $y1 );
	$this->Cell($length,4, "Arrêter le présent Bon de commande à la somme de: ");
	$this->SetXY( $r1 , $y1+5 );

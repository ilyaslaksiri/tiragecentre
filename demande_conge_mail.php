...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($stmt->execute()) {
        ...
        $mail->SMTPAuth = true;
        $mail->Username = 'tiragecentre@gmail.com';
        $mail->Password = 'Tir@ge123////';
        $mail->SMTPSecure = 'tls';
        $mail->Port = 587;

        $mail->setFrom('votre_email@gmail.com', 'Notification Congé');
        $mail->addAddress('ilyaslaksiri@gmail.com'); // Adresse email du destinataire

        $mail->isHTML(true);
        $mail->Subject = 'Nouvelle demande de congé';
        $mail->Body    = "Une nouvelle demande de congé a été soumise par <b>$demandeur</b>.<br>
                          Date de la demande : <b>$date_demande</b><br>
                          Date de début : <b>$date_debut</b><br>
                          Date de fin : <b>$date_fin</b><br>
                          Message : <b>$message</b>";

        if(!$mail->send()) {
            echo 'L\'email de notification n\'a pas pu être envoyé.';
            echo 'Erreur de Mailer: ' . $mail->ErrorInfo;die;
        } else {
            echo 'L\'email de notification a été envoyé avec succès.';die;
        }

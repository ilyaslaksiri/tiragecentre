...
$requete2="select * from bc where ID_BC=$id_bc";
$result2=@mysql_query($requete2, $idcom);
$ligne2=mysql_fetch_array($result2);

$requete="select * from fournisseur where ID_FRS='$ligne2[ID_FRS]'";
...
<form  action="modifier_bc_exec.php" method="post">

<input type="hidden" name="id_frs" value="<?php echo $ligne['ID_FRS'];?>" />
<h4 class="titre_info">Fournisseur : <?php echo $ligne['NOM_FRS'];?></h4>
 
<input type="hidden" name="id_bc" value="<?php echo $ligne2['ID_BC']; ?>" />
 
 <fieldset>
 
<table  width="90%" >
	<tr>			
			<td align="right"> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; Bon de commande N&deg; 		&nbsp;:</td><td>  <?php echo $ligne2['NUM_BC'];?></td>
			<td align="right" > Date 	<span style="color:#FF0000">(*)</span>&nbsp;:</td> <td> <input type="text" name="date_bc" value="<?php echo $ligne2['DATE_BC'];?>" class="required" />  </td>
	</tr>
